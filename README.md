...
This repo is a stop gap.

The package @darron1217/react-native-background-geolocation contains a fix I required on [@maruon85/react-native-background-geolocation](https://github.com/mauron85/react-native-background-geolocation). However, it in itself has an issue. It references another forked package, `background-geolocation-android`, which then references a Maven package that apparently is no longer to be found on Maven: https://github.com/darron1217/background-geolocation-android/blob/develop/VERSIONS.gradle.
...
I could have properly forked that repo and referenced my fork, but Yarn doesn't support submodules. So in order to come full circle I would need to get such a fork merged in _and_ released to NPM. But the `@darron1217/react-native-background-geolocation` package also seems to be behind in NPM releases.

I'll likely make a submission, but in the meantime this repo is essentially `react-native-background-geolocation` copied right out of `node_modules`, with the fix applied to `VERSIONS.gradle`. Because it is now "flat" (no git submodule) this repo can be referenced directly without having to ensure this is submitted to NPM or otherwise use tricks with Yarn to support the submodules.
